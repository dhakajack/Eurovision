...
def search_a_table(table: str, the_cursor) -> bool:
...
trial_terms = ("eudract", "status", "db_date", "title", "nct", "placebo", "condition",
...
while True:
    ...
    if drug_flag:
        final_set &= result_set["drug"]
    if location_flag:
        final_set &= result_set["location"]
    if sponsor_flag:
    ...
    if output_file.casefold() == "a":
        ...
        for trial_selected in sorted(final_set):
            cursor.execute("SELECT {} FROM trial WHERE eudract = \"{}\"".
                           format(trial_term_string, trial_selected))
            ...
            cursor.execute("SELECT {} FROM location WHERE eudract = \"{}\""
                           .format(location_term_string, trial_selected))
            rows = cursor.fetchall()
            location_entry = ", ".join(flatten(rows))
            cursor.execute("SELECT {} FROM sponsor WHERE eudract = \"{}\""
                           .format(sponsor_term_string, trial_selected))
